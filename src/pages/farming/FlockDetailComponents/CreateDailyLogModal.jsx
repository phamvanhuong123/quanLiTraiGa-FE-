...
const CreateDailyLogModal = ({
  ...
  const [materialRows, setMaterialRows] = useState([
    { id: 1, batchId: null, quantityUsed: 1 },
  ]);
  ...
  const fetchAvailableSupplies = async () => {
    ...
    try {
      ...
      console.log("Available supplies from API:", supplies);
  ...
  const addMaterialRow = () => {
    ...
    setMaterialRows([
      ...materialRows,
      { id: newId, batchId: null, quantityUsed: 1 },
    ]);
  };
    ...
    form
      ...
      .then((values) => {
        ...
        const validRows = materialRows.filter(
          (row) => row.batchId && row.quantityUsed && row.quantityUsed > 0,
        );

        if (validRows.length === 0) {
          // Nếu không có vật tư, vẫn cho phép lưu nhật ký
          message.warning(
            "Không có vật tư nào được chọn. Nhật ký sẽ được lưu không có vật tư.",
          );
        }
  ...
  const remainingChickens = flock
    ? flock.currentQuantity - calculateTotalChickens()
    : 0;
        ...
        >
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: "#666", marginBottom: 4 }}>
              <div>
                Mã lô: <span style={{ fontWeight: 500 }}>{item.batchCode}</span>
              </div>
              <div>
                HSD:{" "}
              ...
              <Alert
                ...
                description={
                  ...
                  >
                    <span>
                      Tổng chết/loại:{" "}
                      <strong>{calculateTotalChickens()}</strong>
                    </span>
                    <Tag
                      color={remainingChickens >= 0 ? "green" : "red"}
                      style={{
                        marginLeft: 8,
                        fontSize: 16,
                        padding: "4px 8px",
          ...
          <Col span={12}>
            <Form.Item
              ...
              rules={[
                ...
                {
                  validator: (_, value) => {
                    // Nếu có giá trị và không phải số nguyên thì báo lỗi
                    if (
                      value !== undefined &&
                      value !== null &&
                      ...
                      return Promise.reject(
                        new Error("Vui lòng nhập số nguyên"),
                      );
                    }
                    return Promise.resolve();
                  },
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ marginBottom: 16 }}>
                        ...
                        <Select
                          ...
                          value={row.batchId}
                          onChange={(value) =>
                            handleMaterialChange(row.id, "batchId", value)
                          }
                          style={{ width: "100%" }}
                          ...
                          <Row gutter={16}>
                            <Col span={12}>
                              <div style={{ fontSize: 13, color: "#666" }}>
                                ...
                                <Tag
                                  color={
                                    selectedSupply.type === "FOOD"
                                      ? "green"
                                      : selectedSupply.type === "MEDICINE"
                                        ? "blue"
                                        : "purple"
                                  }
                                  style={{ marginLeft: 4 }}
                              ...
                              >
                                HSD:{" "}
                                {dayjs(selectedSupply.expiryDate).format(
                                  "DD/MM/YYYY",
                                )}
                              </div>
                            </Col>
                            <Col span={12}>
                              ...
                              >
                                Giá:{" "}
                                <span
                                  style={{ fontWeight: 600, color: "#ff7a45" }}
                                >
                                  {selectedSupply.pricePerUnit?.toLocaleString(
                                    "vi-VN",
                                  )}
                                  đ/{selectedSupply.unit}
                                </span>
                              </div>
                            </Col>
                          </Row>
